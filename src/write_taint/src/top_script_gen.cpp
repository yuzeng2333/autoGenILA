...
void top_script_gen() {
  ...
  if(useAllEngine) outFile << "set_engine_mode {Hp Ht Bm J Q3 U L R B K AB D I AD M N AM G C AG G2 C2 Hps Hts Tri}" << el;
  ...
  for(auto it = g_sig2regMap.begin(); it != g_sig2regMap.end(); it++) {
    ...
    if(sig == 0) {
      outFile << "assume -name {a1} {R["+toStr(totalReg-1)+":1] == 0} -update_db" << el;
      outFile << "assume -name {a2} {R[0] == 1} -update_db" << el;
    }
    else if(sig == totalReg - 1) {
      outFile << "assume -name {a1} {R["+toStr(totalReg-2)+":0] == 0} -update_db" << el;      
      outFile << "assume -name {a2} {R["+toStr(totalReg-1)+"] == 1} -update_db" << el;
    }
    else {
      outFile << "assume -name {a1} {R["+toStr(sig-1)+":0] == 0} -update_db" << el;      
      outFile << "assume -name {a2} {R["+toStr(totalReg-1)+":"+toStr(sig+1)+"] == 0} -update_db" << el;
      outFile << "assume -name {a3} {R["+toStr(sig)+"] == 1} -update_db" << el;
      hasThreeAssum = true;
    }
