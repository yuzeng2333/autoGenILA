...
//#include "../src/parse_instr_info.h"
#include "main.h"
...
int main(int argc, char *argv[]) {
  ...
  g_topModule = separate_modules(fileName, modules, childModules, 
                                 totalRegCnt, g_instance2moduleMap);
  read_changed_regs(g_path+"/changed_regs.txt", g_changedRegVec);
  determine_regs_to_check();
  ...
  //if(!g_use_jasper) read_in_instructions(g_path+"/instr.txt");
  ...
  for(std::string subModule: modules) {
    if(subModule.compare(g_topModule) == 0)
      continue;
    output << "`include \"../RTL/" + subModule 
    + "_NEW.v.clean.tainted.final\"" << std::endl;
  }
  assert_reg_map_gen();
  output.close();
