...
int main() {
  ...
  Value* add1 = Builder->CreateAdd(a, c);
  Value* add2 = Builder->CreateAdd(b, d);
  Value* add3 = Builder->CreateAdd(add1, add2);
  ...
  //llvm::Instruction *addInstr = llvm::dyn_cast<Instruction>(add3);
  //BasicBlock* newBB = BB->splitBasicBlock(addInstr, Twine("new_bb"));

  // replace add1 and add2 with memory
  std::string add1Name = add1->getName();
  toCout("add1Name: "+add1Name);
  Builder->SetInsertPoint(instr(add3)->getNextNode());
  AllocaInst* add1_alloca = Builder->CreateAlloca(add1->getType(), nullptr,//addrNum++, 
                                         Twine(add1Name+"_;_mem"));

  //add1_alloca->insertBefore(instr(add1));

  //Value* indexList[1] = {ConstantInt::get(add1->getType(), 0)};
  //auto add1_ptr = Builder->CreateGEP(add1, ArrayRef<Value*>() );
  //auto add1_ptr = Builder->CreateGEP(add1, ArrayRef<Value*>(indexList, 1) );
  //StoreInst* add1_store = Builder->CreateStore(value(add1_ptr), value(add1_alloca));
  StoreInst* add1_store = Builder->CreateStore(add1, value(add1_alloca));
  //add1->replaceAllUsesWith(add1_store);
  //add1_store->insertAfter(add1_alloca);
