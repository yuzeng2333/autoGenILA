...
not need to be modified, but it is sometimes useful to modify a few of the parameters:
...
These files will always contain data for all update functions and ASVs, even if *func_extract* skipped some
update functions due to pre-existing LLVM output.
...
3. The files `func_info.txt` and `asv_info.txt` generated by *func_extract* are read to obtain information about all update functions and ASVs (*sim_gen* does not actually read or write any LLVM data).
...
5. The file `mem.txt` specifies the memory data for a processor-style simulation.  This file is not needed for accelerator-style simulations.  Some designs may also read data memory values from the file `dmem.txt`.
...
A user-created `ila_main.cpp` file can be used.  The easiest way to generate it is to allow *sim_gen* to generate a `ila_main.cpp` file with a skeleton main() function and then manually edit it.  Since the program will not overwrite an existing `ila_main.cpp`, your manual work will not get accidentally destroyed.

After *sim_gen* is run, an execute simulation program can be created by running the `link.sh` script that was created by *func_extract*.  This script will use the Clang compiler and the LLVM linker to compile the C++ code and link it with the LLVM code of each update function.


# The Program *test_gen*

*Test_gen* will read the ILA instruction definitions 
and generate text file with a list of instrucitons to be executed that can be given to *sim_gen*.

## Test_gen Command-Line Options

    test_gen [<path>] [<instr_num>] [<design_opt>] [-verbose]

*test_gen* can often be run without any command-line arguments.  When this is done, the data file path will default to the current directory.
...
* Several other options will adjust test_gen's behavior for specific test cases.  These options include `-aes`, `-pico`, `-urv`, `-vta`, `-other`, and `-non_random`.  The default is `-other`, which is suitable for most accelerator-type designs.  The `-non_random` option will cause the generated instruction list to contain each instruction once, in order.  In this case, the instr_num option is ignored.  By default the generated instruction list will be generated randomly.

## Test_gen Input/Output Files

*test_gen* reads and writes all data files in the directory specified by the `path` option described above.

1. As with *func_extract* and *sim_gen*, the ILA instruction descriptions are loaded from the data file `instr.txt`.

2: The generated instruction list is written to the file `tb.txt`.  This file is meant to be given as input to *sim_gen*.  

3: For some desigs, the file `dmem.txt` will be generated.  This file contains data memory values, and is also meant to be given to *sim_gen*.
