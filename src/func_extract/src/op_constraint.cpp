    ...
    // FIXME: bound+1 or bound+2???
      ...
      if(localVal != "x" && localVal != "DIRTY") {
        if(is_number(localVal)) {
          ...
          toCout("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%B Give "+localVal+" to "+timed_name(dest, timeIdx));
          g_outFile << "Give "+localVal+" to "+timed_name(dest, timeIdx) << std::endl;
          return mixed_value_expr(localVal, c, dest, timeIdx, localWidth-1);
        }
        else {
...
expr two_op_constraint(astNode* const node, uint32_t timeIdx, context &c, solver &s, goal &g, uint32_t bound, bool isSolve) {
  ...
  if(!op2IsNum) {
    expr tmpExpr = add_constraint(node->childVec[1], timeIdx, c, s, g, bound, isSolve);
    if(op2Slice.empty() || has_direct_assignment(op2AndSlice))
      op2Expr = tmpExpr;
    else
      op2Expr = tmpExpr.extract(op2Hi, op2Lo);
  }
  else
