...
void add_node(std::string var, uint32_t timeIdx, astNode* const node ) {
  if ( isInput(var) ) {
...
bool add_child_node(std::string varAndSlice, uint32_t timeIdx, astNode* const node) {
  if( g_ssaTable.find(varAndSlice) != g_ssaTable.end() ) { // if assign exists
    ast* nextNode = new astNode;      
    node->childVec.push_back(nextNode);    
    return add_node(destNext, timeIdx, nextNode);
  }
  else {
    ...
    for(std::string varSlice: reg2Slices[var]) {
        ast* nextNode = new astNode;      
        node->childVec.push_back(nextNode);
        ret &= add_node(varSlice, timeIdx, nextNode);
    }
    return ret;
  }
}
...
bool add_nb_node(std::string regAndSlice, uint32_t timeIdx, astNode* const node) {
  ...
  if(std::regex_match(destAssign, m, pNonblock)) {
    ...
    return add_child_node(destNext, timeIdx+1, node);
  }
...
bool add_ssa_node(std::string varAndSlice, uint32_t timeIdx, astNode* const node) {
...
void add_two_op_node(std::string line, uint32_t timeIdx, astNode* const node) {
  ...
  add_node(op1AndSlice, timeIdx, node);
  add_node(op2AndSlice, timeIdx, node);
...
void add_ite_op_node(std::string line, uint32_t timeIdx, astNode* const node) {
  ...
  add_node(condAndSlice, timeIdx, node);  
  add_node(op1AndSlice,  timeIdx, node);  
  add_node(op2AndSlice,  timeIdx, node);  
}
